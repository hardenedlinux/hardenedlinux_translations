...
4. 此可信监督程序还构成了 BIOS 完整性测定的 RTM、RTS 和 RTR，并且必须保护存储的测定结果（例如，作为启动存储设备上的受保护状态的一部分）并且允许这些结果被可证明地新鲜地报告（例如，以使得它们不能被伪造或者重演）。基于此目的，此监督程序可以使用私密的私钥，例如存储于 BIOS 或者启动存储设备中的私钥
